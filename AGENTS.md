...
- Keep every Skill on the common source skeleton documented in `docs/archive-layout.md`: `SKILL.md`, `README.md`, `agents/`, `scripts/`, `references/`, `LICENSE`, and `.gitignore`; keep tests under `scripts/tests/` when present.
- Runtime archives use `<ArchiveHome>/<source>/<profile>/` plus root-level `archive-profile.json`. Public Skill code must never assume a drive letter, home directory, or current working directory.
