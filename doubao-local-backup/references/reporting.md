...
Run files are stored under `D:\Doubao_Backup\reports\runs\<plan-id>`.
...
Prevent overlapping runs with `D:\Doubao_Backup\tool\auto-backup.lock`. Recover only a lock older than six hours.
